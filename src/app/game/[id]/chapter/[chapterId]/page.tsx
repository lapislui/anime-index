...
export default function GameChapterDetailPage() {
  ...
  const handleSave = async () => {
    try {
      await fetch(`/api/chapters/${params.chapterId}`, {
        method: "PUT",
  ...
  const handleDelete = async () => {
    ...
    try {
      await fetch(`/api/chapters/${params.chapterId}`, { method: "DELETE" });
      router.push(`/game/${params.id}`);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="mx-auto min-h-screen max-w-4xl px-4 py-8">
      ...
      >
        &larr; Back to {chapter.game.title}
      </Link>
      ...
      <div className="space-y-4">
        ...
        <MediaUpload chapterId={chapter.id} onUploaded={fetchChapter} />
      </div>
    </div>
  );
}
