...
export default function NewMoviePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      alert("Failed to create movie entry.");
      setSaving(false);
    }
  };
  ...
  return (
    <div className="mx-auto min-h-screen max-w-2xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold text-foreground">Add New Movie Entry</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <textarea
            ...
            rows={3}
            className="w-full rounded-md border border-border bg-slate-950/50 px-3 py-2 text-foreground placeholder:text-muted focus:outline-none focus:border-accent"
            placeholder="Brief overview of the movie, plot points, etc..."
          />
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          >
            {saving ? "Creating..." : "Create Movie Entry"}
          </button>
          <button
            type="button"
            onClick={() => router.back()}
